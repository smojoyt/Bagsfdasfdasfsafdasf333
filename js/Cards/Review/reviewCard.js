...
card.className = "w-[22rem] h-[20rem] flex flex-col border border-gray-200 bg-white shadow-sm overflow-hidden";
...
<div class="flex items-start justify-between mb-2">
  <div class="flex items-center gap-2 flex-wrap text-sm font-semibold text-gray-900">
    <span class="flex items-center gap-1">
      ${name}${lastInitial}
      <!-- SVG Checkmark -->
      <svg class="w-4 h-4 text-blue-600" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  <div class="text-xs text-gray-500">${date}</div>
    ...
    <!-- Headline -->
    <div class="text-xl font-bold text-gray-900 mb-1 uppercase">${headline}</div>

    <!-- Review Body Text -->
    <p class="text-sm text-gray-700 mb-3 whitespace-pre-line" data-full="${text}">
      ${previewText}
    </p>
  ...
  <div class="flex items-center gap-2 px-4 py-3 border-t border-gray-200 bg-white sticky bottom-0">
    <img src="${productImage}" alt="${productName}" class="w-6 h-6 object-cover border border-gray-300" />
    <span class="text-xs text-gray-600">${productName}</span>
  </div>
`;
